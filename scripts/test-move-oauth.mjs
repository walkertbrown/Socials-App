// Verify the move works via OAuth (acting as the owner). Run after the refresh token is set.
// node --env-file=.env.local scripts/test-move-oauth.mjs
...
console.log("Test photo:", img.name, "| parents now visible:", JSON.stringify(img.parents));
...
try {
  ...
  await drive.files.update({ fileId: img.id, addParents: src, removeParents: dest, fields: "id", supportsAllDrives: true });
  console.log("↩️  Moved it back to the dump.");
} catch (e) {
  console.log("❌ Still blocked:", e.response?.data?.error?.message || e.message);
}
